...
**Finding** an address, sometimes called an address lookup, means obtaining a candidate string. In a business context this is nearly always trivial, because the subject supplies it during onboarding. The interesting cases are the ones where you have a partial record: a company with a registered office but no operating premises, a customer whose statement post is bouncing, a seller whose stated country conflicts with their payout account.
...
| --- | --- | --- | --- | --- |
| Document plus identity binding | 5/5 | The person presenting the address document is the person named on it | Coerced or complicit third party, or a genuine tenant fronting for someone else | Legal obligation under CDD |
...
| Government or registry record | 4/5 | An authoritative body records this address against this person or company | Registry lag after a move, plus access limited or closed in many jurisdictions | Legal obligation, or public register access rules |
| Proof of address document | 4/5 | A named third party sent this person post at this address recently | Edited PDFs and template forgeries, plus stale documents outside the date window | Legal obligation under CDD |
| Credit header or reference data match | 3/5 | A name and address pair appears together in bureau records | Thin file and recently moved users, plus coverage that drops sharply outside major markets | Legitimate interest, permissible purpose under local credit rules |
| AVS check at payment | 2/5 | The billing address matches what the card issuer holds | Stolen card used with the real billing address, plus partial match codes that still let a fraudulent order through | Fraud prevention, legitimate interest |
| Geolocation and IP consistency | 2/5 | The session is consistent, or inconsistent, with the claimed location | Proves the network path, never the residence, and travel produces false alarms | Fraud prevention, legitimate interest |
...
- **Recency.** People move. A bill from six months ago is evidence about a former address.
...
## IP address lookup, and what it actually tells you

An IP address lookup resolves a session's IP to an approximate location, an internet provider, and a flag for traffic that routes through a data center, a VPN, or an open proxy. On the evidence ladder above it sits at 2 out of 5, the same tier as AVS, and for the same reason. It proves the network path, not the person.

Three things an IP address lookup is actually good for: flagging a mismatch between a stated country of residence and the country a session originates from, catching structured fraud where the same handful of IP ranges recur across dozens of otherwise unrelated accounts, and confirming that a payout request is not routing through infrastructure built to hide its origin. iDenfy's own [proxy detection and IP insight](https://www.idenfy.com/proxy-detection-ip-insight/) check is built around that third case, flagging VPN, proxy, and exit node traffic at the session level rather than trying to pin down a residence.

What it cannot do is confirm where someone lives. A frequent traveler, a corporate VPN, or a mobile carrier's shared address pool all produce a location that has nothing to do with the account holder's residence, and a residential proxy defeats the check entirely for anyone motivated enough to buy one. Treat a lookup mismatch as a reason to route the session toward a stronger check on the ladder, never as a decline on its own.
...
- **At the point where money leaves the business:** the strongest check you have. Fraud concentrates on withdrawal and payout, not on registration, and a program that verifies hard at signup and loosely at payout has it backwards.
- **On change of address:** reverify. An address change followed quickly by a payout request to a new destination is one of the highest signal patterns in account takeover, and a flow that lets a user edit their address without any check has a hole in it regardless of how good the onboarding was.
- **Periodically, on risk:** regimes expect refresh on a risk sensitive basis rather than on a fixed clock. Higher risk customers get rechecked sooner.
...
### What's the difference between an address lookup and address verification?

An address lookup returns a candidate address, from a form field, a directory, or a registry search. On its own it says nothing about whether that address is genuinely connected to the person or company you are checking. Verification is the additional step of corroborating the pair against independent evidence, at a strength you could defend to a regulator or a card scheme. A lookup is where the process starts, not where a compliance program should end.
...
### What does an IP address lookup show about someone's address?

It shows whether a session is consistent with a claimed address, not where the person actually lives. An IP address lookup resolves a session to an approximate location, an internet provider, and a flag for VPN, proxy, or data center traffic. Treat a mismatch as a reason to route toward a stronger check, such as a proof of address document, rather than as a decline on its own.
...
To correct a strength rating, a failure mode, or a lawful basis note, edit [methods.yaml](methods.yaml), run `python render_table.py --write`, and open a PR per [CONTRIBUTING.md](CONTRIBUTING.md). The topic outline was adapted from iDenfy's article on how to [find and verify someone's address](https://www.idenfy.com/blog/find-and-verify-someones-address/), which iDenfy publishes. The evidence ladder, the failure modes, and the scope limits above are our own.
